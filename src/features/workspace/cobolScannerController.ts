...
import { VSWorkspaceFolders } from "./workspaceFolders";
import { cobolWorkspaceScanner_ADDFILE, cobolWorkspaceScanner_END_OF_FILE, cobolWorkspaceScanner_KNOWNCOPYBOOK, cobolWorkspaceScanner_SENDCLASS, cobolWorkspaceScanner_SENDENUM, cobolWorkspaceScanner_SENDEP, cobolWorkspaceScanner_SENDINTERFACE, cobolWorkspaceScanner_SENDPRGID, cobolWorkspaceScanner_START_OF_FILE, cobolWorkspaceScanner_STATUS, ScanData, ScanDataHelper } from "./cobolWorkspaceScannerData";
...
import { MakeDep } from "../../extension/commands/makeDependencies";
...
export class VScobolWorkspaceScanner {
    ...
    private static getcobolWorkspaceScannerDirectory(): string {
    ...
    private static async forkScanner(externalFeatures: IExternalFeatures, settings: ICOBOLSettings, sf: ScanData, reason: string, updateNow: boolean, useThreaded: boolean, threadCount: number): Promise<void> {
        const jcobolWorkspaceScannerJS = path.join(VScobolWorkspaceScanner.scannerBinDir, "cobolWorkspaceScanner.js");

        const options: ForkOptions = {
            stdio: [0, 1, 2, "ipc"],
            cwd: VScobolWorkspaceScanner.scannerBinDir,
            env: {
        ...
        const child = fork(jcobolWorkspaceScannerJS, [scannerStyle], options);
        ...
        child.on("message", async (msg) => {
            ...
            if (message.startsWith("@@")) {
                ...
                else if (message.startsWith(cobolWorkspaceScanner_SENDPRGID)) {
                    const args = message.split(",");
                    const tokenName = args[1];
                    const tokenLine = Number.parseInt(args[2], 10);
                    const tokenFilename = args[3];
                ...
                else if (message.startsWith(cobolWorkspaceScanner_SENDCLASS)) {
                    const args = message.split(",");
                    const tokenName = args[1];
                    const tokenLine = Number.parseInt(args[2], 10);
                    const tokenFilename = args[3];
                ...
                else if (message.startsWith(cobolWorkspaceScanner_SENDINTERFACE)) {
                    const args = message.split(",");
                    const tokenName = args[1];
                    const tokenLine = Number.parseInt(args[2], 10);
                    const tokenFilename = args[3];
                ...
                else if (message.startsWith(cobolWorkspaceScanner_SENDENUM)) {
                    const args = message.split(",");
                    const tokenName = args[1];
                    const tokenLine = Number.parseInt(args[2], 10);
                    const tokenFilename = args[3];
                    COBOLWorkspaceSymbolCacheHelper.addClass(tokenFilename, tokenName, tokenLine, TypeCategory.EnumId);
                } else if (message.startsWith(cobolWorkspaceScanner_ADDFILE)) {
                    const args = message.split(",");
                ...
                else if (message.startsWith(cobolWorkspaceScanner_KNOWNCOPYBOOK)) {
                    const args = message.split(",");
                    const enKey = args[1];
                ...
                } else if (message.startsWith(cobolWorkspaceScanner_START_OF_FILE)) {
                    const args = message.split(",");
                    currentFile = args[1];
                    // VSLogger.logMessage(`Starting scan of ${currentFile}`);
                } else if (message.startsWith(cobolWorkspaceScanner_END_OF_FILE)) {
                    const args = message.split(",");
                    currentFile = args[1];
    ...
    private static getScanData(settings: ICOBOLSettings, ws: ReadonlyArray<WorkspaceFolder>, stats: FileScanStats, files: string[]): ScanData {
        ...
        sf.scannerBinDir = VScobolWorkspaceScanner.scannerBinDir;
    ...
    public static async processAllFilesInWorkspaceOutOfProcess(externalFeatures: IExternalFeatures, settings: ICOBOLSettings, viaCommand: boolean, useThreaded: boolean, threadCount: number): Promise<void> {
        ...
        await VScobolWorkspaceScanner.forkScanner(externalFeatures, settings, sf, msgViaCommand, true, useThreaded, threadCount);
