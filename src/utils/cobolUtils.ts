...
import { SourceScannerUtils, COBOLTokenStyle } from "../features/workspace/cobolsourcescanner";
import { cobolRegistersDictionary, cobolStorageKeywordDictionary, getCOBOLKeywordDictionary } from "../keywords/cobolKeywords";
import { VSLogger } from "./logger";
import { VSCOBOLSourceScanner } from "../features/workspace/workspaceSymbolScanner";
import { InMemoryGlobalCacheHelper, InMemoryGlobalSymbolCache } from "../features/workspace/globalcachehelper";
...
import { COBOLFileSymbol } from "../features/workspace/cobolglobalcache";
...
import { VSCOBOLEditorConfiguration } from "../config/workspaceConfiguration";
...
export class VSCOBOLUtils {
    ...
    public static foldTokenLine(text: string, current: cobolSourceScannerInterfaces | undefined, action: FoldAction, foldConstantsToUpper: boolean, languageid: string, settings: ICOBOLSettings, defaultFoldStyle: intellisenseStyle): string {
        let newtext = text;
        const args: string[] = [];
            ...
            switch (action) {
                case FoldAction.PerformTargets:
                    if (current !== undefined) {
                        ...
                        if (actionIt === false) {
                            ...
                            foldstyle = VSCustomIntellisenseRules.Default.findCustomIStyle(settings, argLower, foldstyle);
                        }
                    }
                    break;
                ...
                case FoldAction.ConstantsOrVariables:
                    if (current !== undefined) {
                        ...
                        if (actionIt) {
                            foldstyle = VSCustomIntellisenseRules.Default.findCustomIStyle(settings, arg, foldstyle);
    ...
    public static foldToken(
        ...
        const current: cobolSourceScannerInterfaces | undefined = VSCOBOLSourceScanner.getCachedObject(activeEditor.document, settings);
        if (current === undefined) {
