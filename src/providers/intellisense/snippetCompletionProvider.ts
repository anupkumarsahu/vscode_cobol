...
import { COBOLSourceScanner, SourceScannerUtils } from "../../features/workspace/cobolsourcescanner";
...
class SnippetHelper {
    ...
    protected addSnippet(settings: ICOBOLSettings, kind: CompletionItemKind, snippet: ISimpleSnippet, langId: string, targets: Map<string, CompletionItem[]>): void {
        ...
        let kiSnippet = "";
        const istyle = snippet.alwaysUpperCase ? intellisenseStyle.UpperCase : VSCustomIntellisenseRules.Default.findCustomIStyle(settings, snippet.prefix, settings.intellisense_style);
...
export class SnippetCompletionItemProvider extends SnippetHelper implements CompletionItemProvider {
    ...
    private getCompletionItemForAPI(settings: ICOBOLSettings, langId: string, api: string): CompletionItem | undefined {
        ...
        const iStyle = VSCustomIntellisenseRules.Default.findCustomIStyle(settings, ki.api, settings.intellisense_style);
        switch (iStyle) {
            case intellisenseStyle.CamelCase:
        ...
        for (const [, cis] of targets) {
            for (const ci of cis) {
                if (ci.insertText !== undefined) {
                    ...
                    // Only set range if we found a valid position
                    if (charPosForCall >= 0) {
