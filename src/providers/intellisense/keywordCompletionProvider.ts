...
import { SourceScannerUtils } from "../../features/workspace/cobolsourcescanner";
...
import { KeywordSnippetProvider, SnippetCompletionItemProvider } from "./snippetCompletionProvider";
	...
	private getKeywordsGivenPartialWord(iconfig: ICOBOLSettings, wordToComplete: string, limit: number, langid: string): CompletionItem[] {
		...
		for (const key of words) {
			...
			if (this.onlyShowSnippetKeywords.has(keyLower)) {
				...
				const istyle =  VSCustomIntellisenseRules.Default.findCustomIStyle(iconfig,key, iconfig.intellisense_style); 
				switch (istyle) {
