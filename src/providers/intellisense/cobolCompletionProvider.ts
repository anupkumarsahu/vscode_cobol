...
import { COBOLToken, COBOLVariable, SourceScannerUtils } from "../../features/workspace/cobolsourcescanner";
...
export class COBOLSourceCompletionItemProvider implements CompletionItemProvider {
    ...
    private getPerformTargets(iconfig: ICOBOLSettings, document: TextDocument): TrieSearch {
        const sf: cobolSourceScannerInterfaces | undefined = VSCOBOLSourceScanner.getCachedObject(document, iconfig);
    ...
    private getItemsFromList(iconfig: ICOBOLSettings, tsearch: TrieSearch, wordToComplete: string, kind: CompletionItemKind): CompletionItem[] {
        ...
        for (let c = 0; c < numberOfWordsInResults; c++) {
            ...
            const iStyle = VSCustomIntellisenseRules.Default.findCustomIStyle(iconfig,key.tokenNameLower, iconfig.intellisense_style);
            switch (iStyle) {
...
export { COBOLSourceCompletionItemProvider as CobolSourceCompletionItemProvider };
